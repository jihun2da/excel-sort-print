...
- A4 인쇄용 이미지 생성 전, "표지(0번)" 이름/색상을 입력하면 해당 그룹(시트)의 첫 페이지 맨 앞에
  0번 표지 칸이 추가된다. 기존 데이터는 순서·번호가 전혀 바뀌지 않고 그대로 1번부터 이어지며,
  표지가 추가된 만큼 전체 칸 수가 1개 늘어난다(페이지 수가 늘어날 수 있음).
...
if st.session_state.raw_bytes is not None:
    ...
    if st.session_state.sorted_sheets:
        ...
        if st.session_state.mode == "bulk_image_only":
            ...
            st.caption(
                "그룹마다 이름을 입력하면 해당 그룹의 첫 페이지 맨 앞에 '0번' 표지 칸이 추가됩니다. 기존 "
                "데이터는 순서·번호가 전혀 바뀌지 않고 그대로 1번부터 이어집니다. 비워두면 그 그룹은 표지 "
                "없이 기존처럼 1번부터 생성됩니다."
            )
            ...
            st.caption(
                ...
                "첫 페이지 맨 앞에 '0번' 표지 칸이 자동으로 추가됩니다. 기존 데이터는 순서·번호가 전혀 바뀌지 "
                "않고 그대로 1번부터 이어집니다. 필요하면 이름을 다른 값으로 바꿀 수 있고, 지워서 비워두면 그 "
